...
var ray_tracing_buttons = [];
var ray_tracing_index = undefined;
var ray_tracing_row = undefined;
...
function set_api(index) {
	...
	if(ray_tracing_row != undefined) {
		set_ray_tracing(ray_tracing_index);
		if(api_name == "vk" || api_name == "d3d12" || api_name == "mtl") ray_tracing_row.show();
		else ray_tracing_row.hide();
	}
...
function set_ray_tracing(index) {
	index = Math.min(index, ray_tracing_buttons.length - 1);
	if(ray_tracing_index != index) {
		ray_tracing_buttons[index].button("toggle");
		set_storage("ray_tracing", index);
		ray_tracing_index = index;
	}
}
...
function run(options) {
	...
	command += " -" + api_name;
	if(ray_tracing_index == 0 && (api_name == "vk" || api_name == "d3d12" || api_name == "mtl")) command += " -rt 1";
	...
	// multi-GPU mode
	if(multi_mode_index > 0 && device_1_index > 1 && device_1_items.length > 2) {
...
function init() {
	...
	// ray tracing
	for(let i = 0; i < 2; i++) {
		ray_tracing_buttons[i] = $("#ray_tracing_" + i);
		ray_tracing_buttons[i].on("click", function() { set_ray_tracing(i); });
	}
	ray_tracing_row = $("#ray_tracing_row");
	set_ray_tracing(get_storage("ray_tracing") || 1);
